...
This section covers how to design, implement, and register custom repositories that follow the new Kista model — where generic query capabilities are hidden behind `protected` members of `RepositoryBase<TEntity, TKey>`, and domain-specific queries are exposed through the **Specification Pattern**.
...
| `RepositoryBase<TEntity, TKey>` | Protected query hatch + ready-made query methods |
...
| [Implementation](implementation.md) | How to extend `RepositoryBase<TEntity, TKey>` and use the protected query hatch |
...
// 2. Implement using RepositoryBase
