...
`EntityRepository<TEntity>` exposes its query capabilities through `protected` members on the `Repository<TEntity, TKey>` base class and through extension methods on `IRepository<TEntity>`; the legacy `IQueryableRepository` / `IFilterableRepository` interfaces were removed in 1.7.1.
