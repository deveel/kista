...
## Filter Methods on the Base Class

`Repository<TEntity, TKey>` exposes its filterable, paged, and sorted query capabilities as `protected` members, not through a public capability interface. Subclasses call them directly to implement domain-specific query methods, and consumers reach the same surface through extension methods on `IRepository<TEntity>` (or through a custom interface that wraps them):

```csharp
// In Repository<TEntity, TKey> — protected, available only to subclasses
...
Consumers who resolve `IRepository<TEntity>` get working filter, count, and paged-query methods through the extension methods defined in `RepositoryExtensions` — for example `FindAllAsync(IQueryFilter)`, `CountAsync(IQueryFilter)`, `ExistsAsync(IQueryFilter)`, and the lambda shorthand overloads. The primary access path, however, is through your custom interface's domain-specific methods, which call the protected members above internally.

> The legacy `IFilterableRepository<TEntity, TKey>` capability interface was **removed in 1.7.1**. The `Repository` base class no longer implements it explicitly. If you are upgrading from 1.7.0, see the [Migration from 1.7 guide](../migrating-from-1.7.md).
