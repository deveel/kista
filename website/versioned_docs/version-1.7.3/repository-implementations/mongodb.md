...
- MongoFramework does not natively expose DI integration; the `UseMongoDB<TContext>()` builder extension on `AddRepositoryContext()` (provided by `Kista.MongoFramework`) fills that gap. The legacy `AddMongoDbContext<TContext>()` extension was **removed in 1.7.3** — see [Migrating from 1.7.2](../migrating-from-1.7.2.md).
